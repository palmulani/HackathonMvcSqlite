...
namespace HackathonMvcSqlite.Controllers
{
    ...
    public class InternalTransfersController : Controller
    {
        ...
        public InternalTransfersController(AppDbContext db, InventoryService inv)
        {
            _db = db;
            _inv = inv;
        }
        ...
        {
            var query = _db.InternalTransfers
            ...
            if (warehouseId.HasValue)
                query = query.Where(t =>
                    t.SourceWarehouseId == warehouseId.Value ||
        ...
        public async Task<IActionResult> Create(CancellationToken ct)
        {
            ...
            return View(new InternalTransfer
            {
                Reference = "TRF-" + DateTime.UtcNow.ToString("yyyyMMddHHmm")
            });
        }
        ...
        public async Task<IActionResult> Create(InternalTransfer model, int[] productId, decimal[] quantity, CancellationToken ct)
        {
            ...
            {
                ModelState.AddModelError("", "Source and destination must differ.");
                return View(model);
            }
        ...
        public async Task<IActionResult> AddLine(int internalTransferId, int productId, decimal quantity, CancellationToken ct)
        {
            ...
            if (t == null || t.Status == DocumentStatus.Done)
                return NotFound();
        ...
        {
            var transfer = await _db.InternalTransfers.FindAsync(new object[] { id }, ct);

            if (transfer != null)
            {
                _db.InternalTransfers.Remove(transfer);
                await _db.SaveChangesAsync(ct);
            }
