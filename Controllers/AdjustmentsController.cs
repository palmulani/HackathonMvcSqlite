...
namespace HackathonMvcSqlite.Controllers
{
    ...
    public class AdjustmentsController : Controller
    {
        ...
        public AdjustmentsController(AppDbContext db, InventoryService inv)
        {
            _db = db;
            _inv = inv;
        }
        ...
        public async Task<IActionResult> Create(CancellationToken ct)
        {
            ...
            return View(new StockAdjustment
            {
        ...
        {
            var adj = await _db.StockAdjustments.FindAsync(new object[] { id }, ct);

            if (adj != null)
            {
                _db.StockAdjustments.Remove(adj);
