...
namespace HackathonMvcSqlite.Controllers
{
    ...
    public class ReceiptsController : Controller
    {
        ...
        public ReceiptsController(AppDbContext db, InventoryService inv)
        {
            _db = db;
            _inv = inv;
        }
        ...
        public async Task<IActionResult> Create(CancellationToken ct)
        {
            ...
            {
                Reference = "RCP-" + DateTime.UtcNow.ToString("yyyyMMddHHmm")
            });
        }
        ...
        public async Task<IActionResult> Create(Receipt model, int[] productId, decimal[] quantity, CancellationToken ct)
        {
            ...
            for (var i = 0; i < productId.Length; i++)
            {
                ...
                _db.ReceiptLines.Add(new ReceiptLine
                {
                    ReceiptId = model.Id,
        ...
        public async Task<IActionResult> AddLine(int receiptId, int productId, decimal quantity, CancellationToken ct)
        {
            ...
            if (r == null || r.Status == DocumentStatus.Done)
                return NotFound();
        ...
        {
            var receipt = await _db.Receipts.FindAsync(new object[] { id }, ct);
